;(function() {
    ...
    function initBulkEditCheckboxesForMultiSelectFields () {
        var checkUpperRow = function(input){
        ...
        $cells.children("td:last-child").find(":input").change(function(e){
            checkUpperRow(this);
       });
    }

    function disableMultiSelectFields () {
        $("#availableActionsTable .availableActionMultiSelect select").change(function(e){
            var $this = $(this);
    ...
    skate("shared-item-trigger", {
        ...
        attached: function sharedItemTriggerAttached(element) {
            ...
            InlineDialog(element, target.substring(1), function(contents, trigger, showPopup){
        ...
        "events": {
            "submit": function filterFormSubmitted(element, event) {
                if (!event.defaultPrevented) {
                    var filterName = AJS.escapeHtml(element.elements.filterName.value);

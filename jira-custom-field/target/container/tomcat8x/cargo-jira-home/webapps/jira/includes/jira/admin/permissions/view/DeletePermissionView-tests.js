AJS.test.require('jira.webresources:projectpermissions', function() {
    ...
    test('Should display correct empty value for application role with no display value', function() {
        this.model.set('grants', [
            ANY_LOGGED_IN_USER_GRANT
        ]);

        strictEqual(this.view.$el.find('label').text(), ANY_LOGGED_IN_USER_GRANT.displayName + ' - ' + AJS.I18n.getText('admin.permission.types.application.role.any'),
            'Should display provided empty value for application role with no displayValue');
    });
    ...
    test('Should display correct empty value for group with no display value', function() {
        ...
        strictEqual(this.view.$el.find('label').text(), ANYONE_GRANT.displayName + ' - ' + AJS.I18n.getText('admin.common.words.anyone'),
            'Should display provided empty value for group with no displayValue');
    });
    ...
    test('Should render grant with provided displayValue', function() {
        ...
        strictEqual(this.view.$el.find('label').text(), ADMIN_GROUP_GRANT.displayName + ' - ' + ADMIN_GROUP_GRANT.values[0].displayValue,
            'Should display provided displayValue');
    });

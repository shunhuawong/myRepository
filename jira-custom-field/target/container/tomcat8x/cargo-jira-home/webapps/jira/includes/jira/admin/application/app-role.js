define("jira/admin/application/grouppicker",
[
    "jquery",
    "underscore",
    "backbone",
...
function($, _, Backbone, SingleSelect, GroupDescriptor, ItemDescriptor, contextPath) {
    "use strict";
    ...
    return Marionette.ItemView.extend({
        ...
        _format: function (data) {
            ...
            if (items.length < total) {
                descriptorArgs.label = AJS.I18n.getText("application.access.configuration.groups.partial", items.length, total);
            }
...
define("jira/admin/application/approleeditor",
["jquery", "underscore", "backbone", "jira/skate", "jira/admin/application/grouppicker", "aui/inline-dialog2", "jira/admin/application/application-role-labels"],
function ($, _, Backbone, skate, GroupPicker) {
    "use strict";

    var templates = JIRA.Templates.Admin.ApplicationAccess;
    var upgradeJIRAUrl = WRM.data.claim("com.atlassian.jira.web.action.admin.application-access:upgrade-jira-url");
    var reduceUserCountUrl = WRM.data.claim("com.atlassian.jira.web.action.admin.application-access:reduce-user-count-url");
    var managingGroupsUrl = WRM.data.claim("com.atlassian.jira.web.action.admin.application-access:managing-groups-url");
    var Marionette = Backbone.Marionette;
    ...
    var WarningDialogController = Marionette.Controller.extend({
        ...
        _showWarningDialog: function(warningDialogId) {
            ...
            warningDialog.one("aui-layer-hide", function(e) {
                e.preventDefault();
                warningDialogEl.remove();
            });
            warningDialogEl.show();
        }
    });
    ...
    var ApplicationRoleEditorRowView = Marionette.ItemView.extend({
        ...
        _toggleDefault: function (e) {
            // the following line removes any default group warning dialogs, this is to get around the problem where
            // on firefox, when clicking on the default checkbox, the inline dialog doesn't receive the aui-layer-hide
            // event, and isn't removed
            $("#" + this.warningController.getDefaultGroupWarningId()).remove();

            e.preventDefault();
            this.bus.trigger("toggleDefault", this.model.id);
        },
        _getAppsContainingGroup: function(appRoles, groupName) {
    ...
    _.extend(ApplicationRoleEditor.prototype, Backbone.Events, {
        _toggleDefault: function (defaultKey) {
        ...
        _canShowDefaultGroupWarning: function(groupName) {
        ...
        _addGroup: function (groupName) {
            ...
            if (!existingGroup) {
                ...
                this._setRole(groupNames, this._getDefaultGroupNames()).done(function() {
                    if (this._getApplicationsWhereTheRoleIsDefault(groupName,
                            this._groups.applicationRoleModel.get("appsAndDefaultRoles")).length > 0) {
        ...
        _loadUserCountForGroup: function(groupName) {
            this.IO.getGroupDetails(groupName).pipe(function (groupResponse) {
        ...
        _getApplicationsWhereTheRoleIsDefault: function(groupName, roles) {
        ...
        _setRole: function(groupNames, defaultGroupNames) {
            if (this._active === 0) {
        ...
        _getDefaultGroupNames: function () {
...
define("jira/admin/application/approleseditor",
[
    "jquery",
    "underscore",
    "backbone",
    ...
    "jira/admin/application/approleeditor",
    "jira/admin/application/defaults",
    'wrm/context-path'
],
function(
    $,
    ...
    var errorHandler = function (xhr, reason) {
        ...
        if (reason !== "abort") {
            ...
            if (!xhr || xhr.status === 400) {
                new ErrorDialog({
                    message: AJS.I18n.getText("application.access.configuration.out.of.date"),
                    mode: "warning"
                }).show();
            } else {
    ...
    _.extend(RolesIO.prototype, {
        ...
        getRoles: function () {
            ...
            return this._wrap(this._ajax({
                ...
                dataType: "json"
            }).pipe(function (data, statusText, xhr) {
         ...
         * @returns {jQuery.Deferred}
         */
        putRole: function (roleKey, groups, defaultGroups, selectedByDefault, versionHash) {
            var data = {
            ...
            })).pipe(this._sortGroups);
        },

        putRoles: function(collection) {
            var instance = this;
        ...
        _makeGroupUrl: function (groupName) {
        ...
        _ajaxForPut: function (roleKey, options) {
            if (this._aborted) {
    ...
    var ApplicationRoleModel = Backbone.Model.extend({
        ...
        update: function(data) {
            ...
            return this.collection.IO.putRole(data.key, data.groups, data.defaultGroups, data.selectedByDefault,
                this.collection.versionHash).done(function(result) {
                _.pairs(result).forEach(function(pair) {
                    this.set.apply(this, pair);
                }, this);
            }.bind(this));
        },
        defaultGroupsExistingInAnyOtherRoles: function () {
            var that = this;
    ...
    return function (options) {
        ...
        var applicationDefaults = new ApplicationDefaults(collection);
            ...
            var view = new ApplicationRolesEditorView({
                ...
                buildRoleEditor: function (model) {
                    ...
                    return new RoleEditor({
                        ...
                        IO: IO,
                        setRole: function (groups, defaultGroups) {
                            var roleModel = collection.get({id: model.get("key")});
                            if (roleModel) {
                                ...
                                return updateDfd.pipe(function (roleModelUpdateResult) {
                                    collection.reload();
                ...
                updateEditors: function() {
                    // we need to get snapshot for all applications and update information about seats
                        ...
                        JIRA.trace("role.editors.updated");
                }
            });
        ...
        window.onbeforeunload = function () {
            ...
            if (!result) {
                if (IO.busy()) {
                    if (!IO.sudoVisible()) {
                        result =  AJS.I18n.getText("application.access.configuration.active.ajax");
                    } else {
                        IO.abort();
                    }
                }

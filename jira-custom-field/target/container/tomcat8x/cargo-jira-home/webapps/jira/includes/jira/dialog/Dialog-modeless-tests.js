AJS.test.require([
    ...
    var parametrizedTest = function (name, behaviour, testParameters) {
    ...
    (function shouldCommunicateWithMultipleJIRADialogs() {
        ...
        var buildDialogs = function (bottomDialogBuilder, topDialogBuilder) {
            return {
    ...
    var assertDialogShownWithBlanket = function (shownDialog, hiddenDialogs) {
        assertDialogShownWithBlanketVisibility(shownDialog, hiddenDialogs, true);
    };
    var assertDialogShownWithoutBlanket = function (shownDialog, hiddenDialogs) {
        assertDialogShownWithBlanketVisibility(shownDialog, hiddenDialogs, false);
    };
    var assertDialogShown = function (shownDialog, hiddenDialogs) {
        assertDialogShownWithBlanketVisibility(shownDialog, hiddenDialogs, undefined);
    };
    var assertDialogShownWithBlanketVisibility = function (shownDialog, hiddenDialogs, isBlanketShown) {
    ...
    var assertModelessDialog = function (dialog) {
        assertDialogMode(dialog, "modeless");
    };
    var assertModalDialog = function (dialog) {
        assertDialogMode(dialog, "modal");
    };
    var assertDialogMode = function (dialog, expectation) {
        expectation = ("modal" === expectation ? false : true);
        strictEqual(dialog.isModeless(), !!expectation,
            getDialogTitle(dialog) + " should " + (expectation ? "" : "not ") + "be modeless");
    };


    var assertDialogVisible = function (dialog) {
        assertDialogVisibility(dialog, true);
    };
    var assertDialogHidden = function (dialog) {
        assertDialogVisibility(dialog, false);
    };
    var assertDialogsHidden = function (dialogsHidden) {
        dialogsHidden.forEach(function (dialog) {
            assertDialogHidden(dialog);
        });
    };
    var assertDialogVisibility = function (dialog, expectation) {
        var isVisible;
    ...
    var assertBlanketVisible = function (expectation) {
        var isBlanketVisible = ajsDim && ajsDim.$dim && ("false" === ajsDim.$dim.attr('aria-hidden'));
        strictEqual(!!isBlanketVisible, !!expectation, "blanket should " + (expectation ? "" : "not ") + "be visible");
    };

    var assertDocumentTitle = function (expectation) {
        strictEqual(document.title, expectation, "window title should be equal " + expectation);
    };

    var getDialogTitle = function (dialog) {
        return (dialog.options && dialog.options.windowTitle) ||
            (dialog.get && dialog.get('header') && dialog.get('header')[0].text()) ||
            (dialog.$el && dialog.$el.find('header').text()) ||
            "n/a";
    };
    ...
    var createStackedDialog = function (title, options) {
    ...
    var createStackedAuiDialog = function (title, id) {
        var options = {
            width: 800,
            height: 500,
            id: "dialog-name" + id,
    ...
    var createStackedAuiDialog2 = function (title, id) {
        var options = {
            width: 800,
            height: 500,
            id: "dialog2-name" + id,
        ...
        var dialogTpl = '<section role="dialog" id="' + options.id + '" class="aui-layer aui-dialog2 aui-dialog2-medium" aria-hidden="true"> \
            <!-- Dialog header --> \

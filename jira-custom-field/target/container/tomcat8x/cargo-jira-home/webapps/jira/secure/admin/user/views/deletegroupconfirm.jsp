...
<body>
    <page:applyDecorator id="group-delete" name="auiform">
        ...
        <%--
        Provide a warning if deleting a group that is associated to one or more applications.
        Also provides info about the case where deletion of the current group would cause some number
        of users to lose application access.
        --%>
        <ww:if test="/associatedToAnyApplication == true">
            <%-- this group grants access to one or more applications (application roles) --%>
            <aui:component template="auimessage.jsp" theme="'aui'">
                <aui:param name="'messageType'">warning</aui:param>
                <aui:param name="'iconText'"><ww:text name="'admin.common.words.warning'"/></aui:param>
                <aui:param name="'titleText'"><ww:text name="'admin.deletegroup.application.group.header'"/></aui:param>
                <aui:param name="'messageHtml'">
                    <p>
                    <ww:if test="/rolesForGroup/size > 1">
                        <%-- group associated to multiple apps --%>
                        <ww:text name="'admin.deletegroup.application.group.body.multiple'" />
                        <ul>
                            <ww:iterator value="/rolesForGroup">
                                <li><ww:property value="./name" /></li>
                            </ww:iterator>
                        </ul>
                    </ww:if>
                    <ww:else>
                        <%-- group associated to only 1 app --%>
                        <ww:text name="'admin.deletegroup.application.group.body.singular'"><ww:param name="'value0'" value="/rolesForGroup[0]/name" /></ww:text>
                    </ww:else>
                    </p>
                    <p>
                    <ww:if test="/applicationUsersAffected > 0">
                        <%-- notify # of users that will lose app access because of delete --%>
                        <ww:text name="'admin.deletegroup.application.users.affected'"><ww:param name="'value0'" value="/applicationUsersAffected" /></ww:text>
                    </ww:if>
                    <ww:else>
                        <%-- notify that no users will lose app access because of delete --%>
                        <ww:text name="'admin.deletegroup.application.noone.affected'" />
                    </ww:else>
                    </p>
                </aui:param>
            </aui:component>

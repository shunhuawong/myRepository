AJS.test.require("jira.webresources:jira-setup", function(){
    ...
    module("JIRA SetupFinishing page", {
        ...
        steps: {
            ...
            "pluginsPending": [
                ...
                {key: "plugins", status: "pending"},
                {key: "environment", status: "awaiting"},
                {key: "finishing", status: "awaiting"}
            ],
            "pluginsFailure": [
                {key: "database", status: "success"},
                {key: "plugins", status: "failure"},
                {key: "environment", status: "awaiting"},
                {key: "finishing", status: "awaiting"}
            ],
            "environmentPending": [
            ...
            "environmentFailure": [
                {key: "database", status: "success"},
                {key: "plugins", status: "success"},
                {key: "environment", status: "failure"},
                {key: "finishing", status: "awaiting"}
            ],
            "finishingPending": [
                {key: "database", status: "success"},
            ...
            "finishingFailure": [
                ...
                {key: "plugins", status: "success"},
                {key: "environment", status: "success"},
                {key: "finishing", status: "failure"}
            ],
            "finishingSuccess": [
                {key: "database", status: "success"},
                {key: "plugins", status: "success"},
                {key: "environment", status: "success"},
                {key: "finishing", status: "success"}
            ]
        }
    });
    ...
    test("after setup completes successfully, notifications are replaced with summary view", function(){
        ...
        this.sandbox.server.requests[1].respond(200, {"Content-Type": "application/json"},
            JSON.stringify({steps: this.stepsForResponse(expectedSteps1)}));
        this.assertSteps(expectedSteps1);
        this.sandbox.clock.tick(1);

        this.sandbox.server.requests[2].respond(200, {"Content-Type": "application/json"},
            JSON.stringify({steps: this.stepsForResponse(expectedSteps2)}));
        this.assertSteps(expectedSteps2);
        this.sandbox.clock.tick(1);

        this.sandbox.server.requests[3].respond(200, {"Content-Type": "application/json"},
            JSON.stringify({steps: this.stepsForResponse(expectedSteps3)}));
        this.assertSteps(expectedSteps3);
        this.sandbox.clock.tick(1);

        this.sandbox.server.requests[4].respond(200, {"Content-Type": "application/json"},
            JSON.stringify({steps: this.stepsForResponse(expectedSteps4)}));
        this.assertSteps(expectedSteps4);
        this.sandbox.clock.tick(1);

        this.sandbox.server.requests[5].respond(200, {"Content-Type": "application/json"},
                JSON.stringify({redirectUrl: url}));
    ...
    test("if request times out, a warning is shown to the user", function(){
        ...
        this.sandbox.server.requests[1].respond(200, {"Content-Type": "application/json"},
                JSON.stringify({steps: this.stepsForResponse(this.steps.environmentPending)}));
        ...
        this.sandbox.server.requests[4].respond(200, {"Content-Type": "application/json"},
            JSON.stringify({steps: this.stepsForResponse(this.steps.finishingSuccess)}));
        this.assertSteps(this.steps.finishingSuccess);
        this.sandbox.clock.tick(1);

        this.sandbox.server.requests[5].respond(200, {"Content-Type": "application/json"},
                JSON.stringify({redirectUrl: "whatever"}));

AJS.test.require('jira.webresources:browseprojects', function() {
    require(['underscore', 'backbone', 'jquery'], function(_, Backbone, $) {
        ...
        test('should update UI with selected category', function() {
            ...
            equal(this.$container.find('.category-nav #cat2-panel-tab').hasClass('active'), true);
        ...
        test('should switch pages when clicking on pagination', function() {
            ...
            equal(this.$container.find('.projects-list tr:last td.cell-type-name a').text(), "Project 25");
            ...
            equal(this.$container.find('.projects-list tr:first td.cell-type-name a').text(), "Project 51");
            equal(this.$container.find('.projects-list tr:last td.cell-type-name a').text(), "Project 75");
        });
        ...
        test('should update url with current state', function() {
            ...
            sinon.assert.calledWith(this.navigation.navigate, {selectedCategory: 'all', selectedProjectType: '', contains: false, page: 2});

            this.changeContainsFilter('Project 100');

            sinon.assert.calledTwice(this.navigation.navigate);
            sinon.assert.calledWith(this.navigation.navigate, {selectedCategory: 'all', selectedProjectType: '', contains: 'Project 100'});

            this.$container.find('.category-nav #cat3-panel-tab-lnk').click();
            sinon.assert.calledThrice(this.navigation.navigate);
            sinon.assert.calledWith(this.navigation.navigate, {selectedCategory: 'cat3', selectedProjectType: '', contains: 'Project 100'});

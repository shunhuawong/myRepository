...
AJS.toInit(function ($) {
    ...
    var activeTab = $("meta[name='admin.active.tab']").attr("content"),
        ...
        contextPath = wrmContextPath(),
        inProject = activeSection === "atl.jira.proj.config";


    var adminAnalyticsData = wrmData.claim("jira.webresources:jira-admin-analytics.data");
    ...
    _.defer(function() {
        if (AJS.EventQueue) {
            ...
            JIRA.Page.relatedContentElement().on("click", "a", function() {
                ...
                _.extend(data, adminAnalyticsData);

                AJS.EventQueue.push(adminNavEvent("tabs", data));
            });

AJS.test.require('jira.webresources:projectpermissions', function() {
    ...
    test('Correct empty value shown for application role', function() {
        ...
        strictEqual($grantHTML.find("dd").text(), 'admin.permission.types.application.role.any',
            'Should display provided empty value for application role');
    });
    ...
    test('Correct empty value shown for group', function() {
        ...
        strictEqual($grantHTML.find("dd").size(), 1, 'There should only be one element in the dd');
        strictEqual($grantHTML.find("dd").text(), 'admin.common.words.anyone',
            'Should display provided empty value for group');
    });
    ...
    test('Single values do not have empty values displayed', function() {
        ...
        ok(view.isSingleValueGrant(view.grants[0]),
            'A non-group, non-applicationrole security type with no values passed in is a single value grant');
